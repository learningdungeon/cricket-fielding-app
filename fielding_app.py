...
# This ensures the coach never has to type a number
SQUAD = {
    "David Miller": "34",
    "Shadab Khan": "7",
    "Shaheen Afridi": "10",
...
# --- 2. SIDEBAR (Auto-Lookup) ---
st.sidebar.header("Match Settings")
player_name = st.sidebar.selectbox("Active Fielder", list(SQUAD.keys()))
shirt_no = SQUAD[player_name] # AUTOMATIC LOOKUP

st.sidebar.info(f"Tracking: {player_name} (#{shirt_no})")
...
def add_entry(contact=0, fumble="None", target="None", quality="None", saved=0, given=0, hit=0, availed=0):
    ...
    st.toast(f"Saved: {player_name}")

# --- 4. ACTION GRID (Optimized for Mobile) ---
